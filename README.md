...
[1, 3, 5, 14, 22, 45]
```
#### 3. 당첨 번호 입력 안내 문구를 출력하고, 당첨 번호를 입력받는다.
- 번호는 쉼표(,)를 기준으로 구분한다.
```
당첨 번호를 입력해 주세요.
1,2,3,4,5,6
```
#### 4. 보너스 번호 입력 안내 문구를 출력하고, 보너스 번호를 입력받는다.
```
보너스 번호를 입력해 주세요.
7
```
...
```
#### 6. 수익률을 계산하고, 수익률을 출력한다.
- 수익률은 소수점 둘째 자리에서 반올림한다.
```
총 수익률은 62.5%입니다.
```
#### 7. 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`을 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
